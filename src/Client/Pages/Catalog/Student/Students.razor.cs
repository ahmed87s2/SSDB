...
namespace SSDB.Client.Pages.Catalog.Student
{
    public partial class Students
    {
        ...
        private async Task InvokeModal(string studentNumber)
        {
            var parameters = new DialogParameters();
            if (!string.IsNullOrEmpty(studentNumber))
            {
                var Student = _pagedData.FirstOrDefault(c => c.Id == studentNumber);
                if (Student != null)
                {
                    var StudentModelData = await StudentManager.GetForAddEdit(studentNumber);
                    parameters.Add(nameof(AddEditStudentModal.AddEditStudentModel), StudentModelData.Data);
                }
            }
            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Medium, FullWidth = true, DisableBackdropClick = true };
            var dialog = _dialogService.Show<AddEditStudentModal>(studentNumber == null ? _localizer["Create"] : _localizer["Edit"], parameters, options);
        ...
        private async Task Delete(string studentNumber)
        {
            string deleteContent = _localizer["Delete Content"];
            var parameters = new DialogParameters
            {
                {nameof(Shared.Dialogs.DeleteConfirmation.ContentText), string.Format(deleteContent, studentNumber)}
            };
            ...
            if (!result.Cancelled)
            {
                var response = await StudentManager.DeleteAsync(studentNumber);
                if (response.Succeeded)
                {
                    OnSearch("");
